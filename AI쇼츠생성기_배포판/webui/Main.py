        ...
        with st.container(border=True):
            ...
            with col_coupang_enable:
                enable_coupang = st.checkbox(
                    ...
                    help="영상 설명란과 댓글에 쿠팡 제품 링크를 추가합니다"
                )
            ...
            with col_coupang_links:
                if enable_coupang:
                    ...
                    st.info("💡 쿠팡 파트너스를 활성화하면 제품 링크를 추가할 수 있습니다")
...
if not st.session_state.get("generation_in_progress", False) and 'start_button' in locals() and start_button:
    ...
    if st.session_state.get("enable_coupang_overlay", False):
        try:
            ...
            if product1_name or product1_link:  # 이름이나 링크만 있어도 추가
                products.append({
            ...
            if st.session_state.get("add_product2", False):
                ...
                if product2_name or product2_link:
                    products.append({
            ...
            if st.session_state.get("add_product3", False):
                ...
                if product3_name or product3_link:
                    products.append({
            ...
            if products:
                ...
                logger.info(f"✅ 쿠팡 제품 데이터 생성 완료: {len(coupang_overlay_data)}개")
            else:
                logger.warning("⚠️ 제품 정보가 입력되지 않음")
